...
{
    Init(&dungeonScene, &map, &playerManager, "Ninja", id, sf::Vector2i(32, 32), position, 32*5, 2, 1, 250, 3);

    rect_.setSize(sf::Vector2f(imageWidth_, imageHeight_));
    rect_.setFillColor(sf::Color(255, 0, 255, 30));
...
void EnemyNinja::Update(float delta_time)
{
    if (teleportFlag_)
    {
        ...
        sf::Vector2f moveTo(playerCenter.x + cos(randomAngle) * 80 - 16, playerCenter.y + sin(randomAngle) * 80 - 16);
